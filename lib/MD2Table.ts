...
export class Extractor extends Renderer {
    ...
    get objects(): {}[] {
        return this.extractedTables.map((table) =>
            Extractor.tableToObject(table, this.lowercaseKeys)
        );
    }
        ...
        if (shift !== undefined) {
            ...
            table.forEach((cells) => {
                ...
                cells.forEach((cell, index) => {
                    /* @ts-ignore */
                    rowObj[
                        lowercaseKeys ? keys[index].toLowerCase() : keys[index]
                    ] = cell;
                });
                /* @ts-ignore */
     ...
     */
    protected static createExtractor(
        markdown: string,
        mode?: TableMode,
        lowercaseKeys?: boolean
    ): Extractor {
     ...
     */
    static extractAllObjects(
        markdown: string,
        mode?: TableMode,
        lowercaseKeys?: boolean
    ): {}[] {
        let extractor = Extractor.createExtractor(
            markdown,
            mode,
            lowercaseKeys
        );
    ...
    static extract(
        ...
        return extractor.objects.length > 0
            ? JSON.stringify(extractor.objects[0])
            : null;
    }
